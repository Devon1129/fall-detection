        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Response response) throws IOException {
                ...
                if (response.isSuccessful()) {//回調的方法執行在子線程。
                    ...
                    String name = jsonObject.get("result").getAsString();//"result":"全部監測點"
                    ...
                    for (int i = 0; i < roomsJsonArray.size(); i++) {
                        ...
                        String zoneName = roomsJsonArray.get(i).getAsJsonObject().get("zoneName").getAsString();
                        String roomId = roomsJsonArray.get(i).getAsJsonObject().get("roomId").getAsString();
                        String roomNumber = roomsJsonArray.get(i).getAsJsonObject().get("roomNumber").getAsString();
                        Log.d("fffff", "zoneName: " + zoneName);
                        Log.d("fffff", "roomId: " + roomId);
                        Log.d("fffff", "roomNumber: " + roomNumber);
                        ...
                        JsonArray devicesJsonArray = roomsJsonArray.get(i).getAsJsonObject().get("devices").getAsJsonArray();
                        for (int j = 0; j < devicesJsonArray.size(); j++) {
                            String  deviceId = devicesJsonArray.get(j).getAsJsonObject().get("deviceId").getAsString();
                            String  deviceType = devicesJsonArray.get(j).getAsJsonObject().get("deviceType").getAsString();
                            String  connectFlag = devicesJsonArray.get(j).getAsJsonObject().get("connectFlag").getAsString();
                            String  presence = devicesJsonArray.get(j).getAsJsonObject().get("presence").getAsString();
                            String  datetime = devicesJsonArray.get(j).getAsJsonObject().get("datetime").getAsString();
                            Log.d("fffff", "deviceId: " + deviceId);
                            Log.d("fffff", "deviceType: " + deviceType);
                            Log.d("fffff", "connectFlag: " + connectFlag);
                            Log.d("fffff", "presence: " + presence);
                            Log.d("fffff", "datetime: " + datetime);
